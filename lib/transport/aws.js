    ...
    var api = {
        ...
        get:function(callback){
            ...
            api.s3.listObjects(params, function(err, data) {
                ...
                data.Contents.forEach(function(o) {
                    if (o.Key.indexOf('/', o.Key.length - 1) === -1) { //exclude folders items
                        var sss = {
        ...
        delete:function(req,res,callback){
            ...
            var params = {
                ...
                Key: req.query.prefix + req.params.token //decodeURIComponent(req.url.split('/')[req.url.split('/').length - 1]) // required
            };
        ...
        copy: function (req, res, callback){
            var options = api.options;
            var params = {
                Bucket: options.storage.aws.bucketName, // required
                CopySource: options.storage.aws.bucketName + "/" + req.query.prefix + req.params.name, // required
                //Amazon-s3 backend doesn't like % encoding in data_file_name, Reference: 
                // - https://github.com/thoughtbot/paperclip/issues/343
                Key: req.query.target_prefix + decodeURI(req.query.target_name || req.params.name), // required
                // Avoid access-denied exception, References:
...
function uploadFile(s3, fileName, filePath, opts, callback) {
  ...
  var remoteFilename = opts.prefix + b() + '__' + fileName;
